...
def bm2():
    ...
    adt_data = mu.AnnData(
        X=adt_counts.values.T,  
        obs=pd.DataFrame(index=adt_cells),  
        var=pd.DataFrame(index=adt_meta.index)  
    )


    adt_data.obs['label'] = labels.values.flatten()
    adt_data.obs['batch'] = 2  


    rna_data = mu.AnnData(
        X=rna_matrix.T, 
        obs=pd.DataFrame(index=rna_cells),  
        var=pd.DataFrame(index=rna_meta.index)  
    )


    rna_data.obs['label'] = labels.values.flatten()
    rna_data.obs['batch'] = 2  
