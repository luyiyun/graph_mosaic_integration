...
mdata.obs['label']=mdata.obs['coarse_cluster']
print("Converted AnnData to MuData.")

# 确保 mdata.obs 包含必要的列
if "batch" not in adata.obs:
    raise ValueError("batch information is missing in mdata.obs.")
if "coarse_cluster" not in adata.obs:
    raise ValueError("coarse_cluster (label) information is missing in mdata.obs.")

# 定义参数
num_cell = adata.shape[0]  # 使用的细胞数量
result_dir = "/home/yuyipei/graph_mosaic_integration/result/scmomat_pbmc_benchmark"  # 结果保存目录
