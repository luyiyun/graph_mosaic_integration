...
from gmi.mmAAVI.preprocess import log1p_norm, lsi


def create_mosaic_dataset(root: str, cache_url: str | None = None) -> MuData:
    ...
    atac_df[["chromStart", "chromEnd"]] = atac_df[["chromStart", "chromEnd"]].astype(
        int
    )
...
def preprocess_mudata(mdata: MuData) -> MuData:
    ...
    # NOTE: the varp may not get true dimension after filtering, so we need to set it manually
    mdata.varp["net"] = sp.csr_matrix(net)[:, mask][mask, :]
...
def main():
    ...
    parser.add_argument(
        "--cache_url",
        default=None,
        help="cache url for biothings_client",
    )  # windows not support
    ...
    mdata = create_mosaic_dataset(args.raw_data_dir, cache_url=args.cache_url)
