...
def load_results(result_path):
    ...
    for folder in os.listdir(result_path):
        ...
        if os.path.isdir(folder_path):
            # 读取parameters.json
            param_file = os.path.join(folder_path, 'parameters.json')
...
def plot_results(all_results, save_path='./results'):
    # 计算均值和标准差
    mean_results = all_results.groupby(["num_neg_per_pos", "Metric Type"]).agg(
        mean_value=("GMI", "mean"),
        std_value=("GMI", "std")
    ).reset_index()

    # 图1：Total指标随num_neg_per_pos变化
    total_results = mean_results[mean_results["Metric Type"] == "Aggregate score"]
    ...
    # 图2：多指标变化
    metrics_to_plot = ["Leiden NMI", "Leiden ARI", "KBET", "Aggregate score"]
    plt.figure(figsize=(10, 8))
    for metric in metrics_to_plot:
        metric_results = mean_results[mean_results["Metric Type"] == metric]
...
if __name__ == "__main__":
    result_path = './results'  # 存储结果的文件夹
    save_path = './results'    # 保存图表的文件夹
